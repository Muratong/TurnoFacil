...
function save_cancha(){
		...
		if(!empty($_FILES['img']['tmp_name'])){
			$fname = strtotime(date("Y-m-d H:i"))."_".$_FILES['img']['name'];
			$move = move_uploaded_file($_FILES['img']['tmp_name'], '../assets/imagenes/'.$fname);
			if($move){
				$data .=", imagen = '$fname' ";
			}
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO canchas set ".$data);
		}else{
			$save = $this->db->query("UPDATE canchas set ".$data." where id=".$id);
		}
		if($save)
			return 2;
	}
	function delete_cancha(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM canchas where id = ".$id);
		if($delete)
			return 1;
	}
function save_horario(){
		extract($_POST);
		foreach($hora as $k => $val){
			$data = " id_canchas = '$id_canchas' ";
			$data .= ", hora = '$hora[$k]' ";
			$data .= ", inicio = '$inicio[$k]' ";
			$data .= ", final = '$final[$k]' ";
			$data .= ", precio = '$".$precio[$k]."' ";
			if(isset($check[$k])){
				if($check[$k]>0)
				$save[] = $this->db->query("UPDATE horarios set ".$data." where id =".$check[$k]);
			else
				$save[] = $this->db->query("INSERT INTO horarios set ".$data);
			}
		}
	...
	function reserva_turno(){
		extract($_POST);
		$doc = $this->db->query("SELECT * FROM doctors_list where id = ".$doctor_id);
		$schedule = date('Y-m-d',strtotime($date)).' '.date('H:i',strtotime($time));
		...
		if($doc_sched_check->num_rows <= 0){
			return json_encode(array('status'=>2,"msg"=>"El horario de turno no es válido para el horario del Predio seleccionado."));
			exit;
		}
